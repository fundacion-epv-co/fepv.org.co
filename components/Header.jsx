        ...
        <div className="flex items-center justify-between h-20">
          ...
          <Link href="/" className="flex items-center gap-3 group focus:outline-none" aria-label="Volver a Inicio FEPV">
            ...
            <div className="flex flex-col hidden sm:flex">
              <span className="font-display font-bold text-base text-fepv-dark tracking-tight leading-none">
                Fundación Encuentros
              </span>
              <span className="font-sans text-xs font-semibold text-fepv-green tracking-wider mt-0.5">
                Para la Vida — FEPV
              </span>
            </div>
          </Link>
              ...
              if (hasSubmenu) {
                ...
                return (
                  <div key={link.name} className="relative">
                    ...
                    {/* Dropdown Menu */}
                    {isDropdownOpen && (
                      <div className="absolute left-0 mt-2 w-56 rounded-xl bg-white border border-gray-100 shadow-lg py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-200">
                        {link.submenu.map((subitem) => (
                          <Link
                            key={subitem.name}
                            href={subitem.href}
                            onClick={() => setActiveDropdown(null)}
                            className="block px-4 py-2 text-[14px] text-fepv-gray hover:bg-fepv-light/40 hover:text-fepv-dark transition-colors duration-150"
                          >
                            {subitem.name}
                          </Link>
                        ))}
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-lg text-fepv-dark hover:bg-fepv-light/30 focus:outline-none cursor-pointer"
