...
// Cola de peticiones para evitar rate limiting de Google Sheets
...
function _getCachedData(url) {
  ...
  try {
    const key = `sheets_cache_${url.match(/gid=(\d+)/)?.[1] || url}`;
...
function _setCachedData(url, data) {
...
export async function fetchGoogleSheetData(url, requiredFields = []) {
  ...
  // Intentar servir desde caché primero (carga instantánea)
  const cached = _getCachedData(url);
  if (cached) return cached;

  // Encolar la petición para que se ejecuten de forma secuencial (anti rate-limit)
  const result = new Promise((resolve) => {
    _fetchQueue = _fetchQueue.then(async () => {
      // Doble check del caché (otra petición pudo haberlo llenado)
      const cached2 = _getCachedData(url);
      ...
      try {
        ...
        if (!csvText || csvText.trim().length === 0) throw new Error("CSV vacío desde Google Sheets");
          ...
          Papa.parse(csvText, {
            ...
            complete: (results) => {
              if (!validateCSVStructure(results.data, requiredFields)) {
                if (DEBUG) console.warn(`⚠️ CSV vacío o sin campos requeridos: ${url}`);
              }
              res(results.data || []);
            },
        ...
        _setCachedData(url, parsed);
        resolve(parsed);
      } catch (error) {
        console.warn(`⚠️ Aviso: Google Sheets no respondió (posible rate-limit o 400). URL: ${url}`, error.message);
...
export async function reportConnectionError(failedUrl, errorMessage) {
  try {
    // Ignorar errores de red del cliente (CORS, Adblock, sin internet) para no inundar el correo
    if (errorMessage.includes("Failed to fetch") || errorMessage.includes("Load failed") || errorMessage.includes("NetworkError")) {
      console.warn("Ignorando reporte de error por ser un problema de red del cliente:", errorMessage);
      return;
    }

    // Prevenir múltiples reportes del mismo error en un corto tiempo en el navegador
    if (typeof window !== 'undefined') {
      const cacheKey = `err_${failedUrl}`;
      const lastReport = sessionStorage.getItem(cacheKey);
      if (lastReport && (Date.now() - parseInt(lastReport)) < 1000 * 60 * 60) {
        return; // Ya se reportó hace menos de 1 hora
      }
      sessionStorage.setItem(cacheKey, Date.now().toString());
    }

    const intranetUrl = GOOGLE_APPS_SCRIPT_INTRANET_URL;
    
    let resourceName = "Pestaña Desconocida";
    if (failedUrl.includes("gid=164572338")) resourceName = "Programas_CMS";
    else if (failedUrl.includes("gid=183119704")) resourceName = "Usuarios (Intranet)";
    else if (failedUrl.includes("gid=1002")) resourceName = "Testimonios";
    else if (failedUrl.includes("gid=1489659684")) resourceName = "Agencias_Aliadas";
    else if (failedUrl.includes("gid=0")) resourceName = "Convocatorias";
    else if (failedUrl.includes("gid=2001")) resourceName = "Ofertas (Vacantes)";
    else if (failedUrl.includes("gid=859625685")) resourceName = "Noticias";
    else if (failedUrl.includes("gid=881055830")) resourceName = "Métricas";
    else if (failedUrl.includes("gid=1671239233")) resourceName = "Banner";
    else if (failedUrl.includes("gid=2002")) resourceName = "Galería";
    else if (failedUrl.includes("gid=10001")) resourceName = "Equipo";
    else if (failedUrl.includes("gid=10004")) resourceName = "Preguntas Frecuentes";
    else if (failedUrl.includes("gid=10005")) resourceName = "Metas Donación";
    else if (failedUrl.includes("gid=10006")) resourceName = "Imágenes Programas";
    else if (failedUrl.includes("gid=1634033147")) resourceName = "Imágenes Poblaciones";
    else if (failedUrl.includes("gid=3001")) resourceName = "Configuracion Global";
    else if (failedUrl.includes("gid=1531730366")) resourceName = "Líneas Estratégicas";
    else if (failedUrl.includes("gid=1393253018")) resourceName = "Enfoques";
    else if (failedUrl.includes("gid=1822539549")) resourceName = "Hitos Historia";
    else if (failedUrl.includes("gid=682787246")) resourceName = "Transparencia Fondos";
    else if (failedUrl.includes("gid=140429865")) resourceName = "Participación Detalles";

    const detailedMessage = `No se pudo obtener la información de la pestaña: ${resourceName}\n\nMotivo del fallo: ${errorMessage}\n\nPor favor verifica que la pestaña exista en el archivo de Google Sheets y que el documento siga publicado en la web.`;
...
export async function fetchProgramImagesMap() {
  try {
    ...
    if (data && data.length > 0) {
      data.forEach(row => {
        const progId = (row.programa_id || row.id || "").trim().toLowerCase();
        const url = (row.enlace_imagen || row.imagen || "").trim();
        if (progId) {
          map[progId] = getDirectDriveImageUrl(url);
        }
      });
    }
    return map;
...
export async function fetchPoblacionesImagesMap() {
  try {
    ...
    if (data && data.length > 0) {
      data.forEach(row => {
        const id = (row.id || row.programa_id || row.poblacion_id || "").trim().toLowerCase();
        const url = (row.enlace_imagen || row.imagen || row.url || "").trim();
        if (id) {
          map[id] = getDirectDriveImageUrl(url);
        }
      });
    }
    return map;
